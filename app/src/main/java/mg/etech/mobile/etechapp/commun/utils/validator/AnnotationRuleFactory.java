...
package mg.etech.mobile.etechapp.commun.utils.validator;
...
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.Checked;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.ConfirmPassword;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.DateRule;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.Email;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.IpAddress;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.NumberRule;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.Password;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.Regex;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.Required;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.Select;
import mg.etech.mobile.etechapp.commun.utils.validator.annotation.TextRule;
...
class AnnotationRuleFactory {
	// Debug
	static final String TAG = "AnnotToRuleConverter";
	...
	public static Rule<?> getRule(Field field, View view, Annotation annotation) {
		...
		} else if (DateRule.class.equals(annotationType)) {
			return getDateRule(field, view, (DateRule) annotation);
		}
	...
	private static Rule<View> getDateRule(Field field, View view, DateRule date) {
		if (!TextView.class.isAssignableFrom(view.getClass())) {
			Log.w(TAG,
					String.format(WARN_TEXT, field.getName(),
							Regex.class.getSimpleName()));
		...
		int messageResId = date.messageResId();
		String message = messageResId != 0 ? view.getContext().getString(
				messageResId) : date.message();

		final String moisRegex = view.getContext().getString(R.string.mois);
		String dateRegex;
		if (moisRegex != null) {
			dateRegex = "(0?[1-9]|[12][0-9]|3[01]) (" + moisRegex
					+ ") ((19|20)\\d\\d)";
		} else {
			dateRegex = Rules.REGEX_DATE;
		}

		Log.i("REGEX MOIS", moisRegex + "");
		Log.i("REGEX DATE", dateRegex);
		return Rules.or(message, Rules.eq(null, Rules.EMPTY_STRING),
				Rules.date(message, dateRegex, true));
	}
	...
	private static Rule<Checkable> getCheckedRule(Field field, View view,
			Checked checked) {
